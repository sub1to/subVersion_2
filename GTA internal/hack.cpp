...
bool	CHack::refresh()
{
	...
	CFeat* feat;
	static clock_t	startupClock	= curClock;
	static bool		startup			= true;
	if(startup)
	{
		int64_t	dif			= curClock - startupClock;
		float	mod			= (float) (dif / 10000.f);
	...
	if(!m_entityCleanup.empty() && curClock - m_entityCleanupClock > 0x10)
	{
		...
		{
			std::unordered_map<Entity, int>::iterator it = cleanupCountMap.find(ent);
			if(it != cleanupCountMap.end())
				if(it->second < 0xFF)
					it->second++;
				else
				{
					cleanupCountMap.erase(it);
					m_entityCleanup.pop();
					m_entityCleanupTries = 0;
				}
			else
				cleanupCountMap[ent] = 1;
